...
int iot_device_info_init() {
    ...
    HAL_Snprintf(sg_device_info.client_id, MAX_SIZE_OF_CLIENT_ID,"%s",DEVICEID);
    print_log("clientId:[%s]\n",sg_device_info.client_id);
...
int iot_device_info_set(const char *product_id, const char *device_name) {
    ...
    int ret = HAL_Snprintf(sg_device_info.client_id, MAX_SIZE_OF_CLIENT_ID, "%s", DEVICEID);
    print_log("clientId:[%s]\n",sg_device_info.client_id);
