...
#endif
#endif
    int username_len = strlen(config.username) + 4;
    options->username = (char*)HAL_Malloc(username_len);
    options->password = (char*) HAL_Malloc (31);
    HAL_Snprintf(options->username, 31, "%s", config.username);
    HAL_Snprintf(options->password, 31, "%s", config.password);    
