...
static int _mask_push_pubInfo_to(Qcloud_IoT_Client *c, int len, unsigned short msgId, ListNode **node)
{
    ...
    if ((len < 0) || (len > c->write_buf_size)) {
        err_log("the param of len:[%d] is error! write_buf_size:[%d]\n",len,c->write_buf_size);
    ...
    if (c->list_pub_wait_ack->len >= MAX_REPUB_NUM) {
        ...
        err_log("more than %u elements in republish list. List overflow!\n", c->list_pub_wait_ack->len);
        IOT_FUNC_EXIT_RC(QCLOUD_ERR_FAILURE);
    }
    ...
    if (NULL == repubInfo) {
        HAL_MutexUnlock(c->lock_list_pub);
        err_log("run iot memory malloc is error!\n");
        IOT_FUNC_EXIT_RC(QCLOUD_ERR_FAILURE);
    }
    ...
    *node = list_node_new(repubInfo);
    if (NULL == *node) {
        HAL_MutexUnlock(c->lock_list_pub);
        err_log("run list_node_new is error!\n");
        IOT_FUNC_EXIT_RC(QCLOUD_ERR_FAILURE);
    }
