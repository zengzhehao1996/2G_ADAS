...
int send_mqtt_packet(Qcloud_IoT_Client *pClient, size_t length, Timer *timer) {
    ...
    // print_log("mqtt write::::%s \n",pClient->write_buf);
    print_log("mqtt write buf: len = %d\n[",length);
    /***
    for(int i=0;i<length;i++)
    {
         printk("0x%02x ",pClient->write_buf[i]);
    }
     printk("]\n");
     ****/
    //print_log("sent:::%d length:::%d expired(timer)::%d\n",sent,length,expired(timer));
    istimerexp = !expired(timer);
    if(4==length && pClient->write_buf[0]==0x40 && pClient->write_buf[1]==0x02)
    {
        print_log("send puback .........................timerexp::%d\n",istimerexp);
        istimerexp = TRUE;
    }
    while (sent < length && istimerexp) {
        rc = pClient->network_stack.write(&(pClient->network_stack), &pClient->write_buf[sent], length, left_ms(timer), &sentLen);
        print_log("client network stack write......rc::%d\n",rc);
        if (rc != QCLOUD_ERR_SUCCESS) {
            /* there was an error writing the data */
            break;
        }
        sent = sent + sentLen;
        print_log("send mqtt lenth : %d\n",sent);
    }
...
static int _read_mqtt_packet(Qcloud_IoT_Client *pClient, Timer *timer, uint8_t *packet_type) {
    ...
    if (rc == QCLOUD_ERR_SSL_NOTHING_TO_READ) {
        printk("QCLOUD_ERR_MQTT_NOTHING_TO_READ\n");
    ...
    for (i = 0; i < MAX_MESSAGE_HANDLERS; ++i) {
        if ((NULL != pClient->sub_handles[i].topic_filter)) {
            if (0 == _check_handle_is_identical(&pClient->sub_handles[i], &sub_handle)) {
                ...
                flag_dup = 1;
                Log_e("There is a identical topic and related handle in list! topicName=%s", pClient->sub_handles[i].topic_filter);
                break;
            }
        } else {
            if (-1 == i_free) {
...
static int _handle_publish_packet(Qcloud_IoT_Client *pClient, Timer *timer) {
    ...
    if (QOS1 == msg.qos) {
        print_log("Message QOS1...........msg.id:%d .............!!!!!!!!!!!!!!!!!!!!!!!! ACK.\n",msg.id);
...
int cycle_for_read(Qcloud_IoT_Client *pClient, Timer *timer, uint8_t *packet_type, QoS qos) {
    ...
    switch (*packet_type) {
        ...
        case UNSUBACK:
            ...
            break;
        case PUBLISH: {
            print_log("---------------recv server publish message.\n");
                                   ...
                                   SubTopicHandle *handler, ListNode **node)
{
    ...
    print_log("sub list len:%d\n",c->list_sub_wait_ack->len);
    /* 释放sub列表内存 */
    if(c->list_sub_wait_ack->len > 0)
    {
        unsigned int len = c->list_sub_wait_ack->len;
        ListNode *next;
        ListNode *curr = c->list_sub_wait_ack->head;
        ...
        c->list_sub_wait_ack->len = 0;
        c->list_sub_wait_ack->head = NULL;
        c->list_sub_wait_ack->tail = NULL;
        c->list_sub_wait_ack->match = NULL;
        c->list_sub_wait_ack->free = HAL_Free;
    }
    print_log("sub list len:%d\n",c->list_sub_wait_ack->len);
    ...
    print_log("sub ...... list node new...........\n");
    *node = list_node_new(sub_info);
    if (NULL == *node) {
        HAL_MutexUnlock(c->lock_list_sub);
         print_log("...... list node error...........\n");
