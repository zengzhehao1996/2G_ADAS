...
/* 产品名称, 与云端同步设备状态时需要  */
#define QCLOUD_IOT_MY_PRODUCT_ID            "abc"
/* 设备名称, 与云端同步设备状态时需要 */
#define QCLOUD_IOT_MY_DEVICE_NAME           "def"

#ifdef AUTH_MODE_CERT
    /* 客户端证书文件名  非对称加密使用*/
    #define QCLOUD_IOT_CERT_FILENAME          "abc_cert.crt"
    /* 客户端私钥文件名 非对称加密使用*/
    #define QCLOUD_IOT_KEY_FILENAME           "abc_private.key"
...
#define MQTT_COMMON_TIMEOUT       8000
...
int main(int argc, char **argv) {
	...
	rc = IOT_MQTT_Yield(client, MQTT_COMMON_TIMEOUT);
    ...
    } while (1); //(argc >= 2 && !strcmp("loop", argv[1]));
