...
#######################################
	. ../zephyr-env.sh
CURR_DIR={PWD}
export SMART_BUILD_BOARD="SMART_LINK_V141"
...
# build mcuboot
if [ ! -f mcuboot-v14.bin ]; then
	BOARD=aidong_linde429v14 ./bin/build-boot.sh &&\
		./bin/alg128k mcuboot.bin || die "Build mcuboot"
	mv mcuboot.bin mcuboot-v14.bin
	echo "Build mcuboot OK!"
fi

# build linde_v14
	BOARD=aidong_linde429v14 DEMO=linde ./bin/build-demo.sh || die "Build
	linde_v141 failed" 
	echo "Build signed-linde_v141 OK!"

# check mcuboot
	cat mcuboot-v14.bin signed-zephyr.bin > zephyr.bin || die "Synthetic bin
	packet error!"
