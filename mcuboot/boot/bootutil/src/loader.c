...
boot_status_source(void)
{
    ...
    /* FIXME: If power loss happens during the period of erasing slot0, the 
     * swap state in slot0 will keep the previous state 
     * [image_ok, copy_done, magic okay].  Then the mcuboot will think the 
     * swap has been completed.
     * 
     * FIXUP: check ih_magic additionally to judge the interrupted point.
     */
    hdr = boot_img_hdr(&boot_data, 0);
    if ((state_slot0.magic == BOOT_MAGIC_GOOD) && (state_slot0.copy_done == 1)) {
